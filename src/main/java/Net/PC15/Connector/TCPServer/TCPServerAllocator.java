...
public class TCPServerAllocator {
    ...
    public TCPServerAllocator(INConnectorEvent event) {
        ((ServerBootstrap)((ServerBootstrap)this.TCPServerBootstrap.group(NettyAllocator.GetEventLoopGroup(), NettyAllocator.GetClientEventLoopGroup()).channel(NioServerSocketChannel.class)).option(ChannelOption.SO_BACKLOG, 1024)).childHandler(new TCPServer_ClientChannelInitializer(event, this));
    }
    ...
    public void StopClient(int clientID) {
        ...
        String key = keybuf.toString();
        if (this._ClientMap.containsKey(key)) {
            TCPServer_ClientConnector client = (TCPServer_ClientConnector)this._ClientMap.get(key);
            client.Close();
            this._ClientMap.remove(key);
        }
    ...
    public INConnector SearchClient(TCPServerClientDetail client) {
        ...
        keybuf.append(client.ClientID);
        String key = keybuf.toString();
        return this._ClientMap.containsKey(key) ? (INConnector)this._ClientMap.get(key) : null;
    }
    ...
    public void StopListen(String IP, int Port) {
        ...
        String key = keybuf.toString();
        if (this._ServerMap.containsKey(key)) {
            Channel server = (Channel)this._ServerMap.get(key);
    ...
    public ArrayList<IPEndPoint> getServerList() {
        ...
        if (this._ServerMap == null) {
            ...
            this._ServerMap.entrySet().forEach((value) -> {
                Channel ch = (Channel)value.getValue();
    ...
    public void CheckClientStatus(ExecutorService workService) {
        if (this._ClientMap.size() > 0) {
            this._ClientMap.entrySet().forEach((value) -> {
                INConnector connector = (INConnector)value.getValue();
                workService.submit(() -> {
                    connector.run();
                });
            });
