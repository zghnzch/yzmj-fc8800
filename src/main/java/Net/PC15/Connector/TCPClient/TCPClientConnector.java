...
public class TCPClientConnector extends AbstractConnector {
   ...
   protected ConnectorDetail GetConnectorDetail() {
      try {
         ...
         Logger.getLogger(TCPClientConnector.class.getName()).log(Level.SEVERE, (String)null, var2);
         return null;
      }
   }
            ...
            if (!TCPClientConnector.this._isRelease) {
               if (future.isDone()) {
                  if (future.isCancelled()) {
                     ...
                     future.channel().pipeline().addLast(new ChannelHandler[]{this._tcpClient._Handler});
