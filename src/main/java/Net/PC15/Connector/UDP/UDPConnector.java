...
public class UDPConnector extends AbstractConnector {
   ...
   protected ConnectorDetail GetConnectorDetail() {
      try {
         ...
         Logger.getLogger(UDPConnector.class.getName()).log(Level.SEVERE, (String)null, var2);
         return null;
      }
   }
   ...
   public void channelRead0(ChannelHandlerContext ctx, DatagramPacket UDPmsg) throws Exception {
      if (!this._isRelease) {
         synchronized(this) {
            ByteBuf msg = (ByteBuf)UDPmsg.content();
            ...
            if (!UDPConnector.this._isRelease) {
               if (future.isDone()) {
                  if (future.isCancelled()) {
                     ...
                     future.channel().pipeline().addLast(new ChannelHandler[]{this._UDP._Handler});
